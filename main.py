...
trajectory_log2 = 'models/trajectories2.log'
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        optimizer = tf.train.RMSPropOptimizer(learning_rate=0.001,decay=0.09,momentum=0.9)
        ...
        def traj_step(chars, book, curr, cm):
            ...
            step, input_recu = sess.run(
                [ global_step, RMN.input_recu],
                feed_dict)
        ...
        for epoch in range(n_epochs):
            cost = 0.
            tlog2 = open(trajectory_log2, 'w')
            traj_writer = csv.writer(tlog2)
            ...
            for book, chars, curr, cm in span_data:
                for index in range(len(curr)):
                    ...
                    trajs = traj_step(chars, book, np.expand_dims(curr[index],axis=0), np.expand_dims(cm[index],axis=0))
                    # print (np.array(traj).shape)
                    traj_writer.writerow([bname, c1, c2, index] + list(trajs[0]) )
                    tlog.flush()
